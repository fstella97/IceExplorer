...
class dynamixel:
    ...
    def enable_torque(self, print_only_if_error=False, ID = None):
        ...
        for selected_ID in selected_IDs:
            dxl_comm_result, dxl_error = self.packet_handler.write1ByteTxRx(self.port_handler, selected_ID, ADDR_TORQUE_ENABLE, TORQUE_ENABLE)
    ...
    def disable_torque(self, print_only_if_error=False, ID = None):
        selected_IDs = self.fetch_and_check_ID(ID)
        for selected_ID in selected_IDs:
            dxl_comm_result, dxl_error = self.packet_handler.write1ByteTxRx(self.port_handler, selected_ID, ADDR_TORQUE_ENABLE, TORQUE_DISABLE)
    ...
    def set_operating_mode(self, mode, ID = None):
        ...
        for selected_ID in selected_IDs:
            ...
            if mode in operating_modes:
                ...
                self._print_error_msg("Mode set to " + mode + " control", dxl_comm_result=dxl_comm_result, dxl_error=dxl_error, selected_ID=selected_ID)
